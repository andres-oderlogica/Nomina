...
class boletin
{
  ...
  public function buscar_materias_organizadasxgrado($cod_grado,$corte)
{
  if($corte==1 or $corte==2)
  {
     return $this->buscar_materias_organizadasxgrado_todo_ano($cod_grado);
  }
  else
  {
     return $this->buscar_materias_organizadasxgrado_todo_ano_segundo_semestre($cod_grado);
  }
}

public function buscar_materias_organizadasxgrado_todo_ano_segundo_semestre($cod_grado)
{
  $con = App::$base;
    $sql = "SELECT 
             id_m_o,materia.descripcion as descripcion, tipo
    ...
    $rs = $con->dosql($sql, array($cod_grado));
              ...
              while (!$rs->EOF) 
                   {
                    $resultado[]=array('id_m_o'=>$rs->fields['id_m_o'],'descripcion'=>$rs->fields['descripcion'],'tipo'=>$rs->fields['tipo']);
...
public function buscar_materias_organizadasxgrado_todo_ano($cod_grado)
{
  $con = App::$base;
    $sql = "SELECT 
             id_m_o,materia.descripcion as descripcion, tipo
...
public function buscar_abre_materias_organizadasxgrado($cod_grado,$corte)
{
  if($corte==1 or $corte==2)
  {
     return $this->buscar_abre_materias_organizadasxgrado_todo_ano($cod_grado);
  }
  else
  {
     return $this->buscar_abre_materias_organizadasxgrado_todo_ano_segundo_semestre($cod_grado);
  }
 

}

public function buscar_abre_materias_organizadasxgrado_todo_ano($cod_grado)
{
  
  $con = App::$base;
    $sql = "SELECT 
             id_m_o,materia.abre as descripcion,tipo
...
public function buscar_abre_materias_organizadasxgrado_todo_ano_segundo_semestre($cod_grado)
{
  $con = App::$base;
    $sql = "SELECT 
             id_m_o,materia.descripcion as descripcion, tipo
