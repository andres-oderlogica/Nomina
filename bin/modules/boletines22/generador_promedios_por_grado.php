...
$this->SetFont('Arial','',12);
		...
		//var_dump($promedioC1I);
		//$decimales=intval($promedioC1I);
...
$materiasOrganizadas=$boletin->buscar_abre_materias_organizadasxgrado($cod_grado);
...
for ($i=0; $i < count($estudiantesGrado) ; $i++) { 
	...
	for ($j=0; $j < count($materiasOrganizadas) ; $j++) { 
          ...
          $parcialesCorte=$boletin->identificar_parciales($cortesParaVisualizar-1);
...
$notaFinalMateria[]=$aMostrarCorte[0]['final'];
$aMostrarCorte=""; 
