...
function verCargas2(idgrado)
{
    $.ajax({
        ...
        success: function (data)
        {
            ...
            $('input[name="todos"]').on( 'change', function() {
    if($(this).is(':checked')) {
      $('input[name="orderBox[]"]').prop('checked',true);
      //alert('Se hizo check en el checkbox.');
    } else {
      //alert('Se destildo el checkbox');
      $('input[name="orderBox[]"]').prop('checked',false);
    }
  });
...
function crearPlantillaNotas(id)
{
    ...
    message: "Desea crear la plantilla de notas para este estudiante ?",
    buttons: {
        confirm: {
            label: 'Si',
            className: 'btn-success'
    ...
    $.ajax({  url: "clases/control_crud.php",
              type: "POST",
              dataType: "json",
              data: {opcion:"5",id_estudiante:id
                    },
          })
      .done(function(data){
...
function crear()
{
    ...
    callback: function (result) {
    if(result){
      var checvalues_patrocinadores = new Array();
//recorremos todos los checkbox seleccionados con .each
$('input[name="orderBox[]"]:checked').each(function() {
  //$(this).val() es el valor del checkbox correspondiente
  checvalues_patrocinadores.push($(this).val());
});
  ...
  $.ajax({  url: "clases/control_crud.php",
              type: "POST",
              dataType: "json",
              data: {opcion:"6",valores:checvalues_patrocinadores
                    },
          })
      .done(function(data){
